...
| 来源 | 说明 |
|------|------|
| **本地上传** | 上传 zip/tar/gz/7z 压缩包，支持多文件并行处理 |
| **COS对象存储** | 输入腾讯云COS对象路径，支持单文件和前缀批量下载 |
| **链接下载** | 输入HTTP/HTTPS直链下载，适用于云盘直链等 |

## 分步确认流程

处理过程分为4个步骤，每步可由用户确认后再继续。也可勾选"跳过确认，直接处理"一步到位。

| 步骤 | 内容 | 确认项 |
|------|------|--------|
| 1/4 | 解压并扫描目录 | 目录结构、文件类型统计 |
| 2/4 | 检测文件类型 | 图片/点云/位姿/标定检测结果、标定内容摘要 |
| 3/4 | 并发执行处理 | 查看处理结果和Agent日志 |
| 4/4 | 完成 | 下载结果 |
...
在侧边栏的"COS 配置"中填写腾讯云COS凭证，或在 `config.py` 中预配置：
...
| 工具函数 | 功能 |
|---------|------|
| `download_from_cos` | 从腾讯云COS下载文件 (支持单文件/前缀批量) |
| `download_from_url` | 从HTTP/HTTPS链接下载文件 |
...
2. 选择数据来源（本地上传 / COS / 链接下载）
3. 选择任务模式（Auto/2D/3D/4D）
4. (可选) 勾选"跳过确认，直接处理"
5. (可选) 配置手动路径和字段映射
6. 点击"开始处理"
7. 分步确认模式下，逐步查看并确认每一步结果
8. 下载处理后的结果